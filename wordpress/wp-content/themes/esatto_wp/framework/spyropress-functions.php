...
function spyropress_get_excerpt( $args = '' ) {
    ...
    if ( $link_to_post ) {
        ...
        $text = $text . ' <a class="btn btn-primary" href="' . $permalink . '">' . $link_text . '</a><div class="clear"></div>';
    }
...
function display_slider() {
    ...
    if( get_setting( 'slider_disable' ) ) return;
    if( !is_front_page() ) return;
    spyropress_get_template_part( 'part=templates/slider' );
}
...
function spyropress_wp_nav_menu_objects( $items, $args ) {
    ...
    if( !get_setting( 'menu_home' ) ) {
        $url = is_front_page() ? '#wrapper' : home_url();
        $class = is_front_page() ? ' internal' : ' external';
        $items = '<li class="home-icon' . $class . '"><a href="' . $url . '"><span class="sosa">&#0199;</span></a></li>' . $items;
    }
